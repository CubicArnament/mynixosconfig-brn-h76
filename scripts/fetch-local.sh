#!/usr/bin/env bash
# scripts/fetch-local.sh
#
# bash — парсит вывод fetch-remote.sh, выбирает диск, записывает local-device-paths.nix.
#
# Два режима:
#   1. Локальный: запускает fetch-remote.sh локально через sh
#   2. Удалённый: принимает готовый вывод через env FETCH_RESULT (устанавливается
#      оркестратором fetch-target-device-paths.sh после SSH pipe)
#
# Использование:
#   fetch-local.sh <output-file> [disk-override] [camera-override]
#
# Переменные окружения:
#   FETCH_RESULT         — если задан, использовать вместо локального запуска
...
if [[ ${#DISK_LINES_ARR[@]} -eq 0 ]]; then
  printf "No usable /dev/disk/by-id candidates were detected.\n" >&2
  exit 2
fi
...
if [[ -n "$INSTALL_DISK_FILTER" && -z "$DISK_OVERRIDE" ]]; then
  ...
  case "$FILTER_KIND" in
    system|root)
      ...
      ;;
    *)
      FILTERED=$(printf "%s\n" "${DISK_LINES_ARR[@]}" | awk -F'|' -v needle="$INSTALL_DISK_FILTER" '
        BEGIN { needle_l = tolower(needle) }
        { if (index(tolower($0), needle_l) > 0) print }
      ')
      ;;
  esac
  if [[ -z "$FILTERED" ]]; then
    printf "INSTALL_DISK_FILTER=%s did not match any candidates.\n" "$INSTALL_DISK_FILTER" >&2
    exit 2
  fi
  mapfile -t DISK_LINES_ARR <<< "$FILTERED"
fi

# Предпочесть незанятые диски (если не форсируем system/root)
...
if [[ -n "$DISK_OVERRIDE" ]]; then
  ...
  if ! [[ "$INSTALL_DISK_INDEX" =~ ^[1-9][0-9]*$ ]]; then
    printf "INSTALL_DISK_INDEX must be a positive integer starting from 1.\n" >&2
    exit 2
  fi
  IDX=$(( INSTALL_DISK_INDEX - 1 ))
  if (( IDX >= DISK_COUNT )); then
    printf "INSTALL_DISK_INDEX=%s is out of range for %s candidate(s).\n" \
      "$INSTALL_DISK_INDEX" "$DISK_COUNT" >&2
    exit 2
  fi
  ...
  for (( i = 0; i < DISK_COUNT; i++ )); do
    ...
    printf "  [%s] by-id=%s | path=%s | class=%s | size=%s | model=%s | priority=%s\n" \
      "$(( i + 1 ))" "$byid" "$path" "$class" "$size" "$model" "$priority" > /dev/tty
  done
...
IFS='|' read -r _ DISK DISK_REAL DISK_CLASS DISK_SIZE DISK_MODEL \
  DISK_OCCUPIED DISK_ROOT_BACKING DISK_OCCUPANCY_REASONS \
  DISK_MOUNT_SUMMARY DISK_HOLDER_SUMMARY _ <<< "$SELECTED_LINE"
...
printf "Wrote %s\n" "$OUT"
if (( AUTO_SELECTED && DISK_COUNT > 1 )) && [[ -z "$DISK_OVERRIDE" ]]; then
  printf "  diskDevice = %s  (auto-selected: fastest class, then earliest device order)\n" "$DISK"
else
  printf "  diskDevice = %s\n" "$DISK"
fi
...
printf "  originalCandidateCount = %s\n" "$ORIGINAL_DISK_COUNT"
printf "  filteredCandidateCount = %s\n" "$DISK_COUNT"
printf "  occupancyFilterApplied = %s\n" "$OCCUPANCY_FILTER_APPLIED"
...
if [[ -n "$CAMERA" ]]; then
  ...
  printf "  cameraDevicePath = <not detected, using Nix default>\n"
fi
