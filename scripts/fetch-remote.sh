...
# Не вызывается напрямую: fetch-target-device-paths.sh передаёт этот скрипт
# через SSH pipe: ssh host "sh -s -- [disk-override] [camera-override]"
#
# Вывод: строки вида DISK|...|... и CAMERA|...
set -eu
  ...
  lsblk -dnpo PATH,TYPE,RM,RO,ROTA,TRAN,SIZE,MODEL 2>/dev/null | awk '
    $2 == "disk" && $3 == "0" && $4 == "0" && $6 != "usb" {
      ...
      if (path ~ /\/dev\/nvme[0-9]+n[0-9]+$/ || tran == "nvme") {
        priority = 300; class = "nvme"
      } else if (rota == "0" && (tran == "sata" || tran == "ata")) {
        priority = 200; class = "sata-ssd"
      } else if (rota == "0") {
        priority = 150; class = "solid-state"
      } else if (rota == "1") {
        priority = 100; class = "hdd"
      }
...
emit_disk_candidates() {
  ...
  if [ -z "$lines" ]; then
    printf "No internal non-USB disks found on host.\n" >&2
    exit 2
  fi
